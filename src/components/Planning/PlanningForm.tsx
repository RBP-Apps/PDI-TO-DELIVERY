...
const PlanningForm: React.FC<PlanningFormProps> = ({
  ...
  useEffect(() => {
    if (normalizeStr(formData.itemType) === "bos") {
      // Check cache for packing detail options first
      const cacheKey = "bos_packing_options";
      const cachedOptions = cacheGet(cacheKey);

      if (cachedOptions) {
        setPackingDetailOptions(cachedOptions);
      } else {
        ...
        fetchValues(SHEET1_ID, "'BOS BOM'!E:E")
          .then((values) => {
            if (values.length > 0) {
              ...
              cacheSet(cacheKey, values, 1000 * 60 * 60);
            }
          })
          .catch(() => {
  ...
  useEffect(() => {
    const loadAllMasterItems = async () => {
      ...
      try {
        // Check cache first
        const cacheKey = "all_master_items";
        const cached = cacheGet(cacheKey);
        if (cached) {
          setAllMasterItems(cached);
        ...
        // console.log("[DEBUG] Raw data from Master Items sheet:");
        // console.log("[DEBUG] colA length:", colA.length);
        // console.log("[DEBUG] colB length:", colB.length);
        // console.log("[DEBUG] colC length:", colC.length);

        // Log first few items to see data structure
        for (let i = 0; i < Math.min(5, colA.length); i++) {
          // console.log(
          //   `[DEBUG] Item ${i}: Group="${colA[i]}", Name="${colB[i]}", UOM="${colC[i]}"`
          // );
        }
        ...
        // Cache the result
        cacheSet(cacheKey, allItems, 1000 * 60 * 30); // 30 minutes cache
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (isBOS) {
      // BOS filtering - show filtered products only when packing detail is selected
      ...
      // } else {
      //   // For BOS, show ALL products when no packing detail is selected
  ...
  // Utilities: caching and fetch helpers
  ...
  const loadDropdowns = async (force: boolean = false) => {
    ...
    try {
      // INSTANT CACHE LOAD - should be <100ms
      const cached = cacheGet("dropdown_cache_v1");
      if (!force && cached) {
        setProjectOptions(cached.projects || []);
        setFirmOptions(cached.firms || []);
        setVendorOptionsFlat(cached.vendors || []);
        setStateOptions(cached.states || []);
        setDepartmentOptionsFlat(cached.departments || []);
        setItemTypeOptions(cached.itemTypes || []);
        setItemTypeToVendors(cached.itemTypeToVendors || {});
        setStateToDepartments(cached.stateToDepartments || {});
        setUomOptions(
          cached.uoms || ["Kg", "Pieces", "Meters", "Liters", "Sets"]
        );
        // console.log(
        //   "[PERFORMANCE] ✅ INSTANT CACHE LOAD - Data loaded in <100ms"
        // );
        setDropdownLoading(false);
        return;
      }

      // console.log("[PERFORMANCE] 🔄 Loading fresh data from server...");

      // Phase 1: CRITICAL DATA FIRST (fastest sheets) - OPTIMIZED
      // console.log("[PERFORMANCE] 🚀 Starting dropdown loading...");
      ...
      const [firms, departments, uoms] = await Promise.all([
        ...
        (async () => {
          try {
            ...
            // const secondaryUoms = await fetchValues(
            //   SHEET1_ID,
            //   "'Master Items'!D:D"
            // );
            // if (secondaryUoms.length > 5) {
            //   console.log(
            //     "[UOM] ⚠️ Loaded from secondary source:",
            //     secondaryUoms.length,
            //     "options"
            //   );
            //   return secondaryUoms;
            // }

            // Final fallback
            // const tertiaryUoms = await fetchValues(
            //   SHEET1_ID,
            //   "'Master UOM'!A:A"
            // );
            // console.log(
            //   "[UOM] 🥉 Loaded from tertiary source:",
            //   tertiaryUoms.length,
            //   "options"
            // );
            // return tertiaryUoms.length > 0
            //   ? tertiaryUoms
            //   : ["Kg", "Pieces", "Meters", "Liters", "Sets"];
          } catch (error) {
      ...
      cacheSet("dropdown_cache_v1", {
        ...
        itemTypes,
        uoms,
        itemTypeToVendors: itemTypeToVendorsMapping,
        stateToDepartments: stateToDepartmentsMapping,
      });

      // console.log(
      //   "[PERFORMANCE] ✅ ALL DATA LOADED - Total time should be <3 seconds"
      // );
    } catch (err: any) {
  ...
  const createVendorMapping = async (): Promise<Record<string, string[]>> => {
    try {
      ...
      const maxRows = Math.min(rawItemTypes.length, rawVendors.length, 2000);
      ...
      Object.keys(vendorGroups).forEach((itemType) => {
        result[itemType] = Array.from(vendorGroups[itemType]).slice(0, 50); // Limit vendors per type
      });
  ...
  const createStateMapping = async (): Promise<Record<string, string[]>> => {
    try {
      ...
      Object.keys(departmentGroups).forEach((state) => {
        result[state] = Array.from(departmentGroups[state]).slice(0, 30); // Limit departments per state
      });
  ...
  //         //   const qtySet = field === "qtySet" ? Number(value) : updated.qtySet;
  ...
  //           // For non-BOS items, totalQty should equal qty (qtySet defaults to 1)
  //           if (normalizeStr(formData.itemType) === "bos") {
  //             updated.totalQty = qty * qtySet;
  //           } else {
  //             updated.totalQty = qty; // For non-BOS items, total = qty
  //             if (field === "qty") {
  //               updated.qtySet = 1; // Ensure qtySet is always 1 for non-BOS
  //             }
  //           }
  //         }
  ...
  // Handle item name change with auto-fill UOM for non-BOS items
  ...
  //   // Auto-fill UOM if item name matches master item (only for non-BOS)
  //   if (normalizeStr(formData.itemType) !== "bos") {
  //     const matchingItem = masterItems.find(
  ...
  const submitProductWithRetry = async (
    ...
    while (attempt < maxAttempts) {
      ...
      try {
        ...
        console.warn(
          `[Submit] ❌ Attempt ${attempt} failed for product ${index + 1}: ${
            prod.itemName
          } -> ${message}`
        );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      alert(
        `Critical error during submission: ${
                              ...
                              {normalizeStr(formData.itemType) === "bos" ? (
                                <>
                                  ...
                                  <datalist id={`itemName-${product.id}`}>
                                    {(() => {
                                      // console.log(
                                      //   "[DEBUG] Rendering BOS datalist for product:",
                                      //   product.id
                                      // );
                                      // console.log(
                                      //   "[DEBUG] Current formData.itemType:",
                                      //   formData.itemType
                                      // );
                                      // console.log(
                                      //   "[DEBUG] Current allMasterItems length:",
                                      ...
                                      // console.log(
                                      //   "[DEBUG] BOS Filtered items in datalist:",
                                ...
                                <>
                                  <input
                                    ...
                                    // onFocus={() => {
                                    //   console.log(
                                    //     "[DEBUG] Item name input focused"
                                    //   );
                                    //   console.log(
                                    //     "[DEBUG] Current itemType:",
                                    //     formData.itemType
                                    //   );
                                    //   console.log(
                                    //     "[DEBUG] Current allMasterItems length:",
                                    ...
                                    // onClick={() => {
                                    //   console.log(
                                    //     "[DEBUG] Item name input clicked"
                                    //   );
                                    // }}
                                    list={`itemName-${product.id}`}
                                    className={`px-2 py-1 w-full text-sm rounded border focus:ring-1 focus:ring-blue-500 focus:border-transparent ${
                                      masterItemsLoading
                                  ...
                                  <datalist id={`itemName-${product.id}`}>
                                    {(() => {
                                      // console.log(
                                      //   "[DEBUG] Rendering datalist for product:",
                                      //   product.id
                                      // );
                                      // console.log(
                                      //   "[DEBUG] Current formData.itemType:",
