...
Shipping location-To be delivered at in the state of Punjab, Haryana, Maharastra, Uttar Pradesh & Jharkhand.`
  ...
  const ensureVendorDirectory = async () => {
    if (Object.keys(vendorDirectory).length > 0) return;
  ...
  const handleIndentChange = async (indentNo: string) => {
    ...
    try {
      ...
      // console.log("vendorDirectory", vendorDirectory);
      const info = vendorDirectory[vendorName];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (result.success && result.fileUrl) {
      ...
      if (saveResult.success) {
        ...
        // Optional: log extracted data for downstream use
        try {
          // const mapped = extractProductItemsData();
          // console.log("Mapped Product Items Data:", mapped);
        } catch (err) {
          console.warn("extractProductItemsData failed:", err);
        }
      } else {
        alert(
