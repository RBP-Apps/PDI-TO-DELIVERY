...
const Planning = () => {
  ...
  const loadRows = async (isRetry = false) => {
    setLoading(true);
    setError(null);

    // Check cache first (20 minutes for planning data - moderate update frequency)
    const cacheKey = "planning_indent_data";
    const cached = !isRetry ? cacheGet(cacheKey) : null;
    if (cached) {
      console.log("[Planning] Using cached data");
      setRows(cached);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      cacheSet(cacheKey, filteredData, 1000 * 60 * 20);
